...
| A11 | **Andrew Davidson — helper dev, Command Center access** (founder order, 2026-08-17: "unlock COMMAND CENTER for ANDREW DAVIDSON" / "he is going to be help Dev"; email confirmed by the founder as `andrew.davidson.zenith@gmail.com`, double-OK given). Scope, confirmed: **the Command Center page only**, via the new named allowlist `COMMAND_CENTER_EMAILS` (access.ts) + **manager** membership (`db/sql/0004_add_manager_andrew.sql`, owner applies with `wrangler d1 execute … --remote`). The audit log, investigator, and every `/go/*` handoff **remain founder-only** — deliberately excluded, because the audit log is the founder's record of everyone including helpers. Both the allowlist's exact contents and the helper's refusal at every founder-only surface are pinned by tests. Widening the allowlist is a new A-row, never a code convenience | ✅ recorded 2026-08-17 |
