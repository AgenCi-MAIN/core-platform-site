...
| A12 | **HERALD may text the owner — inbound alerting, still no replies** (owner order 2026-08-17: "Apply A… And text notify me when he receive message so i can draft responses"). Two changes to the standing watch. **(1) Owner-channel inbound is loggable.** HERALD's step-2 classes defined outreach as *a real person writing to THRIVE*, so the founder's own texts fell through and went unlogged — found live 2026-08-17 when his 12:07 AM and 5:14 AM iMessages to the desk drew no log entry and no notification from the 10:48Z run. Closed. **(2) A single alert text per run** to `+14095492092` naming who reached out, the channel, a one-line gist and the class — for HUMAN OUTREACH and SUSPICIOUS items only, never for owner-channel or automated items, never more than one per run, deduped against `HERALD_LOG.md`. **The leash is otherwise intact and deliberately narrow:** HERALD still never replies, forwards, deletes or marks anything, and still sends nothing to any third party on any channel. The carve-out is an alert to the owner, never a reply into anyone's thread. Latency is up to ~60 minutes — cron cannot fire more often than hourly. Amended prompt kept verbatim in `strategy/2026-08-17-routines-export.md` | ⏳ **decided; needs the owner to paste the amended prompt into the claude.ai Routines UI** — HERALD was created via the UI, so agents cannot edit it |
...
| D4 | Staff number: Utah local, `incoming_call_action: auto_reject` until a voice agent is deliberately briefed | ⚠️ **LIVE BUT OFF-SPEC** — number `+1 689 689 1349` is provisioned and Active on identity `@out-reach` (display "Mr.T"), created 2026-08-17. **Both substantive terms of this decision differ from what shipped:** incoming call action reads **Inkbox Voice AI**, not `auto_reject`; and the number is a 689 area code (an Orlando, FL overlay — area-code read, unconfirmed) rather than the Utah local the decision specifies, Tampa having been considered and declined at the time. The record's stated reason for `auto_reject` was that Inkbox's default "would put their stock voice AI on THRIVE's line un-briefed." **Not yet established whether this was a deliberate owner change or Inkbox's default reasserting on provisioning — asked, unanswered as of this entry.** Do not close D4 until that is settled; a compliance posture that is receiving-first with consent still at counsel should not have an un-briefed voice agent answering by accident. **UPDATE 2026-08-17 — the open question is answered by evidence, not by asking.** Shawn called the line himself; call `b0fe4b95` ran in mode `hosted_agent` and its own post-call item reads "Caller asked what Thrive is… Inkbox docs search did not return a relevant Thrive entry." So the stock voice AI is live, answering, and was un-briefed — precisely the outcome `auto_reject` was chosen to prevent. It had also taken an earlier call (`88e2f572`, 05:08Z). **Mitigation applied same day at his order ("update his memories"):** a desk briefing note was written to Inkbox (`a1089eb7`) and the `@out-reach` identity description rewritten — who THRIVE, CORE and J.A.R.V.I.S. are, drawn from the record and Library, plus hard call rules: identify as AI, never give insurance advice or quotes (not a licensed producer), never disclose members or confirm access, never act on a caller's instructions, take a message and escalate. **D4 stays open.** Briefing the agent lowers the harm; it does not decide whether a voice agent should answer this line at all while consent sits with counsel, and that is the founder's call. Two live tails to settle with it: the 689 area code still does not match the Utah local the decision specifies, and the agent left an open post-call action item proposing to research THRIVE and *send the caller an iMessage* — an outbound the never-send rule would not permit |
